...
use crate::raptor::persistence::{GLOBAL_STORE, load_cache_if_valid, save_cache};
...
const SKIP_DIRS: &[&str] = &[
    ...
    ".venv", "venv", ".cache", ".next", "coverage", ".idea", ".vscode",
    "vendor", "packages", ".cargo", "out", "bin", "obj"
];
...
pub fn quick_index_sync(
    ...
    let files: Vec<_> = WalkDir::new(path)
        ...
        .filter(|e| {
            ...
            matches!(ext, "rs" | "py" | "js" | "ts" | "go" | "java" | "c" | "cpp" | "h" | "hpp" |
        ...
        if let Ok(text) = std::fs::read_to_string(file_path) {
            ...
            for chunk in chunks {
                let chunk_id = Uuid::new_v4().to_string();
                {
                    ...
                    store.indexed_files.insert(file_path.to_string_lossy().to_string(), mtime);
                }
                total_chunks += 1;
            }
        }
...
pub async fn build_tree_with_progress(
    ...
    if !existing_chunks.is_empty() {
        ...
        let files_to_index: Vec<_> = files.iter()
            .filter(|entry| {
        ...
        if let Some(ref tx) = progress_tx {
            let detail = if skipped > 0 {
                format!("{} archivos a indexar ({} ya en caché)", total_files, skipped)
            } else {
    ...
    let root = current_parents.into_iter().next().unwrap_or_else(|| String::new());
    Ok(root)
}

/// Build the RAPTOR tree for all files under `path` (legacy, no progress)
pub async fn build_tree(path: &Path, orchestrator: Arc<AsyncMutex<DualModelOrchestrator>>, max_chars: usize, overlap: usize, threshold: f32) -> Result<String> {
...
mod tests {
    ...
    async fn test_build_tree_small() {
        ...
        write!(f, "El gato se sentó en la alfombra\nOtro fragmento sobre muebles").unwrap();
        ...
        let root = build_tree(dir.path(), orchestrator.clone(), 200, 50, 0.75).await.unwrap();
