...
impl<'a> TreeRetriever<'a> {
    ...
    async fn retrieve_with_emb(
        ...
        let mut results: Vec<(String, f32, String)> = heap
            .into_iter()
            .filter_map(|Reverse((score, idx))| {
                ...
                self.store
                    .nodes
                    .get(id)
                    .map(|node| (id.clone(), score.into_inner(), node.summary.clone()))
            })
            .collect();
    ...
    pub async fn retrieve_with_context(
        ...
        let mut chunk_matches: Vec<(String, f32, String)> = heap
            .into_iter()
            .filter_map(|Reverse((score, idx))| {
                ...
                self.store
                    .chunk_map
                    .get(id)
                    .map(|text| (id.clone(), score.into_inner(), text.clone()))
            })
            .collect();
...
mod tests {
    ...
    async fn test_retriever_basic() {
        ...
        store.insert_chunk(
            "c1".to_string(),
            "Un fragmento sobre gatos y muebles".to_string(),
        );
        ...
        let (_summaries, chunks) = retriever
            .retrieve_with_context("gatos y muebles", 3, 3, 0.95)
            .await
            .unwrap();
    ...
    async fn test_retriever_batching() {
        ...
        let (summaries, chunks) = retriever
            .retrieve_with_context("gatos y muebles", 5, 10, 0.99)
            .await
            .unwrap();
